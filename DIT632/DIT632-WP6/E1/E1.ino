...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(ENCA), ISR_readEncoder, CHANGE); //interrupt for ENC A pin
...
int getAction(int error){
  u_out = kp * e; //calculting u_out as function of the error and the kp
...
void ISR_readEncoder(){
  a = digitalRead(ENCA); //read enc a signal
  b = digitalRead(ENCB); //read enc b signal
  
  //these values are correct only with interrupt on the enc A pin
  if(a == 1 && b == 0){ //check rotation direction
  pos++; //rotating CW so we increase pos
  }else if(a == 1 && b == 1){ //check rotation direction
  pos--; //rotating CCW so we decrease pos
  }
}