...
const int red = 5; //pin for red LED
const int ringpin = 2; //pin for light ring
int temperature; //storage for light calculation 
...
void setup()
{
  ...
  Timer1.attachInterrupt(callback); //callback function when the timer threshold is hit.
}
void loop(){}
void callback()
{
  ...
  //-20 C to 40 C
  if (celsiusTemp > 40){ //checks if temp is above our decided max temperature also makes sure our loop cant go above the limit of the ring
    temperature = 12; //sets to 12 as this corresponds to 12 lights after conversition
  	digitalWrite(red, HIGH); //turn on LED as temperature is out of bounds
  }
  else{
  temperature = celsiusTemp + 20; //makes sure our decided temperature is in range for calculation(lowest temp -20)
  temperature = temperature / 5; //gives us a value from 0-12 that represents the amount of lights needed to be turned on.
  digitalWrite(red, LOW); //turn off LED as temperature is within bounds
  }
  for(int i=0; i<temperature; i++) //loops through all lights that should be turned on by the given temperature and turns them on
  {
    ring.setPixelColor(i, 100, 55, 255); //sets lights to pretty purple color
  }
  for(int z=temperature; z<12; z++) //loops through the lights not touched by previous loop and turns them off in case if temperature goes down
  {
    ring.setPixelColor(z, 0, 0, 0); //turns lights off
  }
